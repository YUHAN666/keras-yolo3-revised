...
from keras.layers import Conv2D, Add, ZeroPadding2D, UpSampling2D, Concatenate, MaxPooling2D, SeparableConv2D, Activation
...
#
#
#     x3, y1 = make_last_layers(x3, 512, num_anchors*(num_classes+5))
#
#     x3 = compose(
#             DarknetConv2D_BN_Leaky(256, (1, 1)),
#             UpSampling2D(2))(x3)
#     x2 = Concatenate()([x3, x2])
#     x2, y2 = make_last_layers(x2, 256, num_anchors*(num_classes+5))
#
#     x2 = compose(
#             DarknetConv2D_BN_Leaky(128, (1, 1)),
#             UpSampling2D(2))(x2)
#     x1 = Concatenate()([x2, x1])
...
def yolo_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    '''Get corrected boxes
       将yolo_head输出的pred_box在feature_map上的 x,y,w,h转换为在原图上的 x1,y1,x2,y2
    '''
    box_yx = box_xy[..., ::-1]
    box_hw = box_wh[..., ::-1]
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    grid_shapes = [input_shape//{0: 32, 1: 16, 2: 8}[l] for l in range(num_layers)] # [input_shape//32, input_shape//16, input_shape//8]
    y_true = [np.zeros((m, grid_shapes[l][0], grid_shapes[l][1], len(anchor_mask[l]), 5+num_classes),
        dtype='float32') for l in range(num_layers)]

    # Expand dim to apply broadcasting.
    anchors = np.expand_dims(anchors, 0)
    anchor_maxes = anchors / 2.  # anchor 右下点坐标
    anchor_mins = -anchor_maxes  # anchor 左上点坐标
    ...
    for b in range(m):
        ...
        box_maxes = wh / 2.     # gt_box 右下点坐标
        box_mins = -box_maxes   # gt_box 左上点坐标
        ...
        for t, n in enumerate(best_anchor):
            for l in range(num_layers):  # 最佳anchor所在层数
                if n in anchor_mask[l]:
                    i = np.floor(true_boxes[b,t,0]*grid_shapes[l][1]).astype('int32')  # 最佳anchor在所在featuremap层的横坐标
                    j = np.floor(true_boxes[b,t,1]*grid_shapes[l][0]).astype('int32')  # 最佳anchor在所在featuremap层的纵坐标
...
def yolo_loss(args, anchors, num_classes, ignore_thresh=.5, print_loss=False):
    ...
    for l in range(num_layers):
        ...
        raw_true_xy = y_true[l][..., :2]*grid_shapes[l][::-1] - grid  # gt_box相对于每一个grid的偏移
        ...
        # loss += confidence_loss + class_loss
        if print_loss:
